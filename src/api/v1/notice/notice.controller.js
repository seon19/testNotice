...
export const getNoticeById = async (req, res) => {
  try {
    const notice = await NoticeService.getNoticeById(req.params.id); // ID로 게시판 가져오기
    if (!notice) {
...
export const createNotice = async (req, res) => {
  try {
    ...
    if(!_id || !title || !content) {
        return console.log("필수 값이 입력되지 않았습니다.");
    }
    const Notice = await Notice.create({
        _id, title, content
    });
    const newNotice = await NoticeService.createNotice(req.body); // 새 게시판 생성
...
export const updateNotice = async (req, res) => {
  try {
    const updatedNotice = await NoticeService.updateNotice(req.params.id, req.body); // 지정한 ID의 게시판 업데이트
    if (!updatedNotice) {
      return res.status(404).json({ message: "해당 게시판을 찾을 수 없습니다." });
    }
...
export const deleteNotice = async (req, res) => {
  try {
    const result = await NoticeService.deleteNotice(req.params.id); // 게시판 삭제
    if (!result) {
      return res.status(404).json({ message: "해당 게시판을 찾을 수 없습니다." });
    }
